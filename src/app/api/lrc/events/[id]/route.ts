...
export async function PATCH(
  ...
  const fields = [
    ...
    'event_date', 'end_date', 'registration_url', 'status', 'is_featured',
  ...
  if (data?.slug) revalidatePath(`/learn/events/${data.slug}`)
...
export async function DELETE(
  ...
  const { id } = await params
  const { error } = await supabase.from('lrc_events').delete().eq('id', id)
